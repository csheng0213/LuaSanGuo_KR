...
function VipData:RequestVipPay(puchaseToken,paymentSeq,productCode,handler,isResend)
    ...
    local onServerRequest = function (event)
        ...
        for i = 1,#g_payProductConfig_Gplay do 
            if event.productCode == g_payProductConfig_Gplay[i].productCode then
                Functions.setAdbrixTag("buy",tostring(g_payProductConfig_Gplay[i].money),tostring(PlayerData.eventAttr.m_level))  
            end
        end   
