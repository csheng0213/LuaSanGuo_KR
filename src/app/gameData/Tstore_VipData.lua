...
function VipData:RequestVipPay(nonce,paymentSeq,productCode,handler,isResend)
    ...
    local onServerRequest = function (event)
        ...
        for i = 1,#g_payProductConfig_Tstore do 
            if event.productCode == g_payProductConfig_Tstore[i].productCode then
                Functions.setAdbrixTag("buy",tostring(g_payProductConfig_Tstore[i].money),tostring(PlayerData.eventAttr.m_level))  
            end
        end   
...
function VipData:needToConsumProduct()
    Functions.callJavaFuc(function( )
        if GameState.storeAttr.curProductCode_s ~= "" then 
            VipData:RequestVipPay(0,GameState.storeAttr.paymentSeq_s,GameState.storeAttr.curProductCode_s,nil,true) 
        end 
    end)    
end
