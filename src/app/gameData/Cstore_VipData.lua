...
function VipData:RequestVipPay(nonce,paymentSeq,productCode,handler,isResend)
    ...
    local onServerRequest = function (event)
        ...
        for i = 1,#g_payProductConfig_Cstore do 
            if event.productCode == g_payProductConfig_Cstore[i].productCode then
                Functions.setAdbrixTag("buy",tostring(g_payProductConfig_Cstore[i].money),tostring(PlayerData.eventAttr.m_level))  
            end
        end   
